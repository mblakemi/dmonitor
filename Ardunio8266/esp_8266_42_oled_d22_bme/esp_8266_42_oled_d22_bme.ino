...
#define UseBME      // must also have BMP180 and UseBMP280

float dTempOffset = 0.0;
...
float fPressOffset = 0.18; // calc .15826 Pressure offset was .98
#else
#include <Adafruit_BMP085.h>
Adafruit_BMP085 bmp;
float fPressOffset = .16; // Pressure offset
#endif
...
#define IP_LAST_OCTET 42 // This defines this node's static IP, read on for more info (also find array NODEMCU_NODES)
...
// --------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  // Start the server
  Serial.println("Pre server started");
  server.begin();
...
//  BMP 180 or 280
  ...
  display.setFont(ArialMT_Plain_16);
  sText = sText = "P=" + pressStr + " T=" + floatToStr(fTempF, 1);
