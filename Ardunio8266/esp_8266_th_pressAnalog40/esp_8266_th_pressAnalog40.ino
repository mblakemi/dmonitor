...
// 18/18/18 set up for Bentley WiFi
...
const IPAddress myIp(192, 168, 0, IP_LAST_OCTET);
...
codeus::NODEMCU_NODE NODEMCU_NODES[] = {
  {false,  40, "bOffice", NULL}, //08/26/2018 changed from true
  {false, 62, "Office", NULL},
};
  ...
  if (request.length() > 0) { // we have a request
    ...
    if (request.indexOf("/reset") >= 0) { // this allows you to reset a node
...
#endif
#ifdef ANALOG
      String strAnalog = String(fLastAnalogAvg);
      fullAnalogStr = "[dark[" + strAnalog + "]]";
#endif
      ...
      payload = "[[temperature[" + tempStr + "]][[humidity[" + humStr + "]][name[" + nodeName + "]][time[" + timeStr + "]][date[" + dateStr + "]]"
        + fullPressStr + fullRainStr + fullAnalogStr +  "[uptime[" + (millis() / 1000) + "]]";			
...
#endif
#ifdef ANALOG
        "<br><font size='16' color='black'>Dark: " + strAnalog + "</font><br>" +
#endif
...
void collectSlaveData() {
  ...
  for (int i = 0; i < NODEMCU_NODE_COUNT; i++) {
    ...
    if (node->lastIPOctet != IP_LAST_OCTET) { // skip ourselves
      String url = "http://141.133.76." + String(node->lastIPOctet) + ":" + PORT + "/data";
...
// Update Analog
#ifdef ANALOG
float fAnalogMaxAvg = 0.0;
unsigned long previousAnalogMillis = 0;
unsigned long analogInterval = 2500;

int nAnalogCount = 10;  // check time many times to determine max
int iAnalog = 0;
// for last 4
float fLastAnalogVal[] = {0.0, 0.0, 0.0, 0.0};
int nLastAnalogTot = sizeof(fLastAnalogVal)/sizeof(*fLastAnalogVal);
int iLastAnalogMod = 0;
float fAnalogMax = 0.0; // current max within iAnalogCount
...
void updateAnalog() {
  ...
  if(currentMillis - previousAnalogMillis >= analogInterval) { // read only at given interval, not every time in the loop
    // save the last time you read the sensor 
    previousAnalogMillis = currentMillis;

    float fain = analogRead(A0);
    if (iAnalog < nAnalogCount) {
      if (fAnalogMax < fain) {
          fAnalogMax = fain;
      }
      //fSum += fain;
      ...
      //fLastAvg = fSum/iSum;
      iLastAnalogMod = (iLastAnalogMod + 1) % nLastAnalogTot;
      fLastAnalogVal[iLastAnalogMod] = fAnalogMax;
      float fTempSum = 0.0;
      for (int ii=0; ii< nLastAnalogTot; ++ii) {
        fTempSum += fLastAnalogVal[ii];
      }
      // Update global value
      fLastAnalogAvg = fTempSum/nLastAnalogTot;
      Serial.print("ALastMax= ");
      Serial.print(fLastAnalogAvg);
      Serial.println("  T= " + tempStr+ "  H= "+ humStr + "%");
      // Reset 
      // Reset Values
      // iSum = 1;
      fAnalogMax = fain;
